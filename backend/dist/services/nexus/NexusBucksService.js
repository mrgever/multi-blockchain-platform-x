...
exports.NexusBucksService = void 0;
class NexusBucksService {
    users = new Map();
    ...
    createUser(identifier, type = 'wallet') {
    ...
    // Purchase packages
    purchasePackage(userId, packageId, paymentMethod) {
    ...
    purchaseCustomAmount(userId, amount, paymentMethod) {
        ...
        let bonus = 0;
        if (amount >= 1000)
            bonus = Math.floor(amount * 0.25); // 25% bonus
        else if (amount >= 500)
            bonus = Math.floor(amount * 0.20); // 20% bonus
        else if (amount >= 200)
            bonus = Math.floor(amount * 0.15); // 15% bonus
        else if (amount >= 100)
            bonus = Math.floor(amount * 0.10); // 10% bonus
    ...
    // Spend N-Bucks
    spendNBucks(userId, amount, description, category) {
        const user = this.users.get(userId);
        if (!user) {
            return { success: false, message: 'User not found' };
        }
    ...
    earnNBucks(userId, amount, description, source) {
        const user = this.users.get(userId);
        if (!user) {
            return { success: false, message: 'User not found' };
        }
    ...
    calculateDailyActivity(transactions) {
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            const dayTransactions = transactions.filter(t => t.timestamp.toDateString() === date.toDateString());
            last7Days.push({
    ...
    getUserAchievements(user, transactions) {
        const achievements = [];
        if (user.totalEarned >= 1000)
...
exports.NexusBucksService = NexusBucksService;
//# sourceMappingURL=NexusBucksService.js.map