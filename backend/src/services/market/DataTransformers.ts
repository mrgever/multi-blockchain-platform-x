...
export class DataTransformers {
  ...
  static cmcTrending(data: any): any {
    if (!data?.data) return { coins: [] };
    
    return {
      coins: data.data.map((coin: any, index: number) => ({
        item: {
          id: coin.slug || coin.symbol?.toLowerCase(),
          name: coin.name || '',
          symbol: coin.symbol || '',
  ...
  static cryptoCompareTrending(data: any): any {
    ...
    return {
      coins: data.Data.slice(0, 10).map((coin: any, index: number) => {
        const coinInfo = coin.CoinInfo || {};
        return {
          item: {
            id: coinInfo.Name?.toLowerCase() || '',
  ...
  // Helper methods
  private static generateSparkline(basePrice: number): number[] {
    const data = [];
  ...
  private static getMultiplier(value: string): number {
    if (!value) return 1;
  ...
  static getTransformers(endpoint: string): Record<string, (data: any, params?: any) => any> {
    const transformerMap: Record<string, Record<string, (data: any, params?: any) => any>> = {
      'coins/markets': {
      ...
      'coins/list': {
        ...
        'CoinMarketCap': (data: any) => [],
        'CryptoCompare': (data: any) => []
      }
    };
